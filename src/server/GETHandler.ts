    ...
    public static async handle(req: IncomingMessage, res: ServerResponse, solid_server_url: string, query_registry: QueryRegistry, endpoint_queries: EndpointQueries, latest_minutes: number) {
        ...
        if (req.url !== undefined) {
            ...
            if (endpoint === '/averageHRPatient1') {
                query_registry.register_query(endpoint_queries.get_query('averageHRPatient1', new Date(from_timestamp), new Date(to_timestamp)), query_registry, from_timestamp, to_timestamp);
            }
            else if (endpoint === '/averageHRPatient2') {
                query_registry.register_query(endpoint_queries.get_query('averageHRPatient2', new Date(from_timestamp), new Date(to_timestamp)), query_registry, from_timestamp, to_timestamp);
            }
